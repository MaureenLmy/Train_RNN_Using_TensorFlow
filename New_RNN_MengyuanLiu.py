...
cell = tf.contrib.rnn.LSTMCell(num_units=hidden_dim, activation=tf.nn.sigmoid)
#cell = tf.contrib.rnn.GRUCell(num_units=hidden_dim, activation=tf.nn.sigmoid)
...
#loss = tf.reduce_sum(tf.losses.mean_squared_error(Y_, h_), name='loss')
